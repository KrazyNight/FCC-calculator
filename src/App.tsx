...
function App() {
  ...
  const buttonPress = (symbol: string) => {
    if (symbol === "clear") {
      ...
      //if last number has decimal, don't add another
  ...
  const calculate = () => {
    // if last char (character) is an operator, do nothing
    if (isOperator(et.charAt(et.length - 1))) return;
    // clean  expression so that two operators in a row use the last operator
    // 5 * - 5 =
